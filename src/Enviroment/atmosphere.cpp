...
void Atmosphere::print_to_log(std::ofstream&ofs, quantity<m>& geometric_altitude) const{
    ...
    || mp_units::abs(pressure - 26493.5 * Pa) > eps_P
    || mp_units::abs(density - 0.413484 * kg/m3) > eps_rho
    || mp_units::abs(sonic_velocity - 299.505 * m/s) > eps_v) {
    return false;
    }

    update(50000 * m);
    
    if (mp_units::abs(temperature - delta<K>(270.65)) > eps_T
    || mp_units::abs(pressure - 79.7647 * Pa) > eps_P
    || mp_units::abs(density - 0.001026 * kg/m3) > eps_rho
    || mp_units::abs(sonic_velocity - 329.77 * m/s) > eps_v) {
    return false;
    }
